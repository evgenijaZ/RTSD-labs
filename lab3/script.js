...
function FFT(array, N){
	...
	for (var p = 0; p < N; p++) {
		...
		var im = 0;
		 for (var k = 0; k < N; k++) {
			 re += array[k][1] * Math.cos(2 * Math.PI * p * k / N);
			 im += array[k][1] * Math.sin(2 * Math.PI * p * k / N);
		 }
		 result[p][1] = Math.sqrt(re * re + im * im);
	}
	return result;
}

function FN_FFT(fn_arr, fft_arr){
	var N = fn_arr.length;
	var result = [];
    for (var j = 0; j < N; j++)
        result.push([j, 0]);
	for (var p = 0; p < N; p++) {
		result[p][1]=fn_arr[p][1];
		result[p][2]=fft_arr[p][1];
	}
	return result;
}
